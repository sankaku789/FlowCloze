...
quota_rs = r'''//! Provider quota settings and request pacing.
...
fn validate_profile(profile: &QuotaProfile) -> Result<(), String> {
    if profile.rpm == Some(0) {
        return Err(format!("quota profile '{}' の rpm は1以上にしてください", profile.name));
    }
    if profile.tpm == Some(0) {
        return Err(format!("quota profile '{}' の tpm は1以上にしてください", profile.name));
    }
    if profile.rpd == Some(0) {
        return Err(format!("quota profile '{}' の rpd は1以上にしてください", profile.name));
    }
...
impl RateGate {
    ...
    pub fn acquire(&self, estimated_tokens: u64) -> Result<Duration, String> {
        ...
        loop {
            let now = Instant::now();
            let mut state = self.inner.state.lock().map_err(|_| "quota rate gate lock failed")?;
            while state
                .recent
                .front()
...
'''
Path("src/quota.rs").write_text(quota_rs)
...
replace_once(
    ...
    "let identity_plan = match prepare_selected_plan(&scaffold, &identity_indexes, options.policy, None) {",
)
replace_once(
    "src/orchestration.rs",
...
text = Path("src/planner.rs").read_text()
text = text.replace(
    "            policy.batch_policy.max_concurrent_batches,",
    "            effective_batch_policy.max_concurrent_batches,",
)
Path("src/planner.rs").write_text(text)

planner = Path("src/planner.rs")
text = planner.read_text()
start = text.index("    while let Some(attempt) = retry_queue.pop() {")
end = text.index("\n\n    if !terminal_failures.is_empty()", start)
retry_loop = r'''    let mut retry_batch_number = initial_batch_total;
    while !retry_queue.is_empty() {
        let pending = std::mem::take(&mut retry_queue);
        let retry_batches = plan_retry_attempts(
            scaffold,
            pending,
...
planner.write_text(text[:start] + retry_loop + text[end:])
...
marker = "fn validation_error_id(error: &ValidationError) -> String {"
planner = Path("src/planner.rs")
text = planner.read_text()
idx = text.index(marker)
helpers = r'''fn quota_adjusted_batch_policy<E>(
    scaffold: &ScaffoldDocument,
    base: BatchPolicy,
    quota: Option<&QuotaProfile>,
    estimator: &E,
    ...
    E: TokenEstimator,
{
    ...
    while batch_count > request_budget {
        ...
        if policy.max_estimated_input_tokens < hard_tokens {
            ...
            let step = remaining.min(1_000).max(1);
...
'''
planner.write_text(text[:idx] + helpers + text[idx:])

planner = Path("src/planner.rs")
planner.write_text(planner.read_text() + r'''
...
Path("config.toml.example").write_text(r'''# Copy this file to ~/.config/flowcloze/config.toml
provider = "gemini"
...
# Select a quota profile to enable quota-aware batch planning and request pacing.
# The Gemini values below are an example for a 5 RPM / 250K TPM / 20 RPD project;
# adjust every value to the limits shown for your own project/tier.
quota_profile = "gemini"
...
# Model-specific values override the provider profile when present.
# [quota_profiles.gemini.models."gemini-3.8-flash"]
# rpm = 5
# tpm = 250000
# rpd = 20

# OpenAI and Mistral use the same quota schema. Fill in the values for your tier,
# then set quota_profile = "openai" or "mistral" above.
[quota_profiles.openai]
reserve_requests = 0
adaptive_max_tasks_per_batch = 12
adaptive_max_input_tokens = 18000
# rpm = 500
